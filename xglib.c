...
#include "xglib.h"
 ...
 void vAgregaVentana (displayX *psdXA, windowX *pswXA)
 {
   ...
   pswXA->pswXSig = 0;
   if (!(psdXA->pswXFin)) psdXA->pswXIni = pswXA;
   else psdXA->pswXFin->pswXSig = pswXA;
   psdXA->pswXFin = pswXA;
 }
 ...
 { /*Regresa el manejador de la ventana*/
   ...
   pswX = psdX->pswXIni;
   while (pswX)
     if (pswX->iId==iVentana) return (pswX);
     else pswX = pswX->pswXSig;
   return (0);   
 }
       ...
       if (XCheckWindowEvent(ObtenDisplay(psdX), ObtenVentanaV(pswX), pswX->lMask, &sXE))
       {
         switch (sXE.type)
         {
           case Expose:
             ...
             if (sXE.xexpose.count==0) vReDraw (pswX->iId);
           break;
           case ButtonPress:
           case ButtonRelease:
             //bSalir = TRUE;
 ...
 { /*Retorna el primer tipo de evento*/
   ...
   while (pswX)
   {
     if (XCheckWindowEvent(ObtenDisplay(psdX), ObtenVentanaV(pswX), pswX->lMask, &sXE)) return (sXE.type);
     pswX = pswX->pswXSig;
   }
   return (0);
 }
