...
int main () {
    ...
    for (y=0; y<FIN; y++) {
        for (x=0; x<(psM->iNodos); x++) {
            ...
            if (psM->psnNodos[x].iY>AREA) {
                psM->psnNodos[x].fA = psM->psnNodos[x].fA * -1;
            }
            if (psM->psnNodos[x].iY<0) {
                psM->psnNodos[x].fA = psM->psnNodos[x].fA * -1;
            }
            if (psM->psnNodos[x].iX>AREA) {
                psM->psnNodos[x].fA = PI - psM->psnNodos[x].fA;
            }
            if (psM->psnNodos[x].iX<0) {
                psM->psnNodos[x].fA = PI - psM->psnNodos[x].fA;
            }
            vColorForeground (psdX, psw1, PAR);
            XFillRectangle (ObtenDisplay(psdX), ObtenBufferV(psw1), ObtenGCV(psw1), psM->psnNodos[x].iX-2, psM->psnNodos[x].iY-2, 4, 4);
            if (iObtenKey(psdX)==KEY_ESC) {
                y = FIN;
            }
