...
char cCreaEnlace (matriz *psmM, int i, int j, double fW) {
    ...
    pseEj->pseAnt = 0;
    pseEj->pseSig = 0;
    if (!(psmM->psnNodos[i].pseIni)) {
        psmM->psnNodos[i].pseIni = pseEi;
    } else {
        psmM->psnNodos[i].pseFin->pseSig = pseEi;
    }
    pseEi->pseAnt = psmM->psnNodos[i].pseFin;
    psmM->psnNodos[i].pseFin = pseEi;
    if (!(psmM->psnNodos[j].pseIni)) {
        psmM->psnNodos[j].pseIni = pseEj;
    } else {
        psmM->psnNodos[j].pseFin->pseSig = pseEj;
    }
...
char cEsSimetrica (matriz *psmM) {
    ...
    for (x=0; x<(psmM->iNodos); x++) {
        ...
        while (pseE) {
            if (cHayEnlace(psmM, x, (pseE->iNodo))!=3) {
                cRet = 0;
            }
            pseE = pseE->pseSig;
        }
